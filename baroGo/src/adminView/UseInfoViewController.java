...
public class UseInfoViewController implements Initializable {
	...
	public void handleBtnUseInfoViewAction(ActionEvent action) throws Exception {
		ViewerUtil.showStage(this, "../useInfo/useInfoPopUP.fxml", "../useInfo/useInfo.css", new UserInfoPopUP());
	}
	...
	public void handleMenuTimeAddAction(ActionEvent action) {
		...
		String pcNumber = menuId.substring(menuId.length() - 1, menuId.length());
	...
	public void handleItemTimeAdd(ActionEvent aciotn, String pcNumber, int addTime) {
		...
		Platform.runLater(()->{
			...
			btnOK.setOnAction(event->{
			    String sumTime = db.getAddTime(strID, addTime);
			    db.updateRemaintimeAndPcNumber(sumTime, pcNumber);
			...
			lblAlert.setText("'"+ strID + "' 님에게 "+ addTime +"시간을 추가하시겠습니까?");
